...
function svgAlerteRecap(taille: number, couleur: string): string {
  return `<svg width="${taille}" height="${taille}" viewBox="0 0 24 24" fill="none" stroke="${couleur}" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"><path d="m21.73 18-8-14a2 2 0 0 0-3.48 0l-8 14A2 2 0 0 0 4 21h16a2 2 0 0 0 1.73-3Z"></path><line x1="12" y1="9" x2="12" y2="13"></line><line x1="12" y1="17" x2="12.01" y2="17"></line></svg>`;
}
  ...
  for (const jeune in grouped) {
    ...
    grouped[jeune].forEach((emprunt) => {
      ...
      const alertTag = isOverdue
        ? `<span style="display:inline-flex; align-items:center; gap:3px; background: #ffebee; color: #d32f2f; font-weight: bold; border: 1px solid #d32f2f; padding: 2px 6px; border-radius: 4px; font-size: 10px; margin-right: 6px;">${svgAlerteRecap(10, "#d32f2f")}RETARD</span>`
        : "";
